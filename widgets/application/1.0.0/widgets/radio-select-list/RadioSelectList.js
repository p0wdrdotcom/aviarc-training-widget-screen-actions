...
(function () {
    ...
    YAHOO.namespace("quickLoanCalculator");
    var quickLoanCalculator = YAHOO.quickLoanCalculator;
    var L = YAHOO.lang;
    var REFRESH = {};
    var $ = YAHOO.jQuery_1_9_1_application;

    quickLoanCalculator.RadioSelectList = function () {
        this.onSelectedChanged = new Toronto.client.Event("RadioSelectList onSelectedChanged");
        this.onValueChanged = this.onSelectedChanged;
        this.onEnabledChanged = new Toronto.client.Event("RadioSelectList onEnabledChanged");
        this.onValidChanged = new Toronto.client.Event("RadioSelectList onValidChanged");
        this.onMandatoryChanged = new Toronto.client.Event("RadioSelectList onMandatoryChanged");
        this.onFocus = new Toronto.client.Event("RadioSelectList onFocus");
        this.onOpen = new Toronto.client.Event("RadioSelectList onOpen");
        this.onClose = new Toronto.client.Event("RadioSelectList onClose");
    };

    YAHOO.lang.extend(quickLoanCalculator.RadioSelectList, Toronto.framework.DefaultWidgetImpl, {

        startup: function (widgetContext) {
            quickLoanCalculator.RadioSelectList.superclass.startup.apply(this, arguments);
            ...
            this._onChangeEvent = widgetContext.createManagedEvent("RadioSelectList onchange ManagedEvent");
        ...
        bind: function (dataContext) {
            quickLoanCalculator.RadioSelectList.superclass.bind.apply(this, arguments);
        ...
        _bindFieldEvents: function(enable){
            ...
            if((enable || YAHOO.lang.isUndefined(enable)) && row !== null){
                ...
                this._validHandler = metadata.onValidChanged.bindHandler(this._validChanged, this, 'RadioSelectList');
                this._readOnlyHandler = metadata.onReadOnlyChanged.bindHandler(this._readOnlyChanged, this, 'RadioSelectList');
                this._mandatoryHandler = metadata.onMandatoryChanged.bindHandler(this._mandatoryChanged, this, 'RadioSelectList');
        ...
        refresh: function () {
            ...
            var $selectListInput = $(this.getNamedElement('input')); // so we can get the options and thier data
            var $containerElement = $(this.getContainerElement());
            
            var $containerHolder =  $( '<div class="dotContainer"/>' ); // container to hold the option divs
            $containerHolder.append($('<div class="bar"/>')); // create the background bar
            $containerElement.append($containerHolder);
            var options = $selectListInput.children();
             
            var boxWidth = 16; // we need to know ahead of time the widths of the boxes
            var numberOptions = options.length;
            var parentWidth = $containerHolder.width();
            var space = (parentWidth - (boxWidth * numberOptions)) / (numberOptions -1); // given the size and number of boxes work out the margin
            ...
            for (var i = 0; i < numberOptions; i++) {
                 ...
                 if ((i+1)!==numberOptions){
                    $option.css("margin-right",space+"px");
                 }
                 $option.text($selectOption.text());
                 $option.data('option', $selectOption.val()); // attach the data of the select option value for later
                 $dot = $('<div class="dot"/>');
                 $indicator = $('<div class="indicator"/>')
                 $dot.append($indicator);
                 
                 if ($option.data('option') === this.getValue()){ // are we currently selected
                 ...
                 (function(option) {
                        $(option).click(function(){
                            $containerHolder.find('.indicator').removeClass("selected");
                            $(this).find('.indicator').addClass("selected");
                            me.setValue($(this).data('option'));// manually move the select list option based on the data
                            });
                 })($option);
                 
                 
                 $containerHolder.append($option);  // stick me on the screen now.
    ...
    YAHOO.lang.augmentProto(quickLoanCalculator.RadioSelectList, Toronto.util.CssUtils);
